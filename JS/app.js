...
const rederCategories = () => {
  ...
  categories.forEach((category) => {
    // get all the tasksof current category
    ...
    div.addEventListener("click", () => {
      ...
      calculateTotal();
      // remender task when category cahnge
      renderTasks();
    })
    div.innerHTML = `
    <div class="left">
...
const renderTasks = () => {
  ...
  if (categoryTasks.length === 0) {
    tasksContainer.innerHTML = '<p class="no-task">no task for this category</p>'
  }

  else {
    // if there are task for selected category render them
